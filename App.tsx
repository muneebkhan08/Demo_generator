...
const App: React.FC = () => {
  ...
  const handleGenerate = async (url: string, instructions: string, files: File[]) => {
    ...
    try {
      ...
      const html = await bringToLife(url, instructions, images);
      
      if (html) {
        const newCreation: Creation = {
          id: crypto.randomUUID(),
          name: url ? new URL(url).hostname : (instructions.slice(0, 20) || 'Untitled Demo'),
          html: html,
          // Store the first image as the thumbnail/reference if available
          originalImage: images.length > 0 ? `data:${images[0].mimeType};base64,${images[0].base64}` : undefined,
          timestamp: new Date(),
        };
      ...
      alert("Something went wrong while generating the demo. Please try again.");
    } finally {
